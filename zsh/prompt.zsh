...
if [[ "$(tput colors)" == "256" ]]; then
    ...
    # some custom colors
    fg[magic_blue]=$FG[105]
    fg[magic_red]=$FG[196]
    fg[magic_orange]=$FG[214]
    fg[magic_lavender]=$FG[129]
    fg[magic_lavender_two]=$FG[105]
    fg[magic_git]=$FG[075]
...
function PR_DIR() {
    ...
    echo "%{$fg[magic_blue]%}${truncated}%{$fg[magic_git]%}%B${last}%b%{$reset_color%}"
}

# An exclamation point if the previous command did not complete successfully
function PR_ERROR() {
    echo "%(?.%{$fg[magic_lavender_two]%}✓ .%(!.%{$fg[violet]%}.%{$fg[magic_lavender]%})%B%?%b%{$reset_color%} )"
}
...
function PR_ARROW() {
    echo "%(!.%{$fg[red]%}.%{$fg[blue]%})${PR_ARROW_CHAR}%{$reset_color%}"
}
...
function RPR_USER() {
    if [[ "${RPR_SHOW_USER}" == "true" ]]; then
        echo "%(!.%{$fg[red]%}.%{$fg[grey]%})%B%n%b%{$reset_color%}"
    fi
}

RPR_SHOW_TIME=true
function RPR_TIME() {
    if [[ "${RPR_SHOW_TIME}" == "true" ]]; then
        [[ $(date "+%H") -gt 16 ]] && local hours='%{$fg[magic_red]%}' || local hours='%{$fg[magic_blue]%}' 
        #local time_color="%(hours > 12 || hours < 15.%{$fg[green]%}.$fg[grey])"
        #echo "%{$time_color%}
        echo "$hours%T%{$reset_color%} "
    fi
}
